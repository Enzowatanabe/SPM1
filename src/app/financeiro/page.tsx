...
export default function FinanceiroPage() {
  ...
  useEffect(() => {
    const salvo = localStorage.getItem('lancamentos');
    if (salvo) {
      const parsed: Lancamento[] = JSON.parse(salvo);
      setLancamentos(parsed);
      calcularSaldo(parsed);
    }
  }, []);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const listaNova = [...lancamentos, novo];
    setLancamentos(listaNova);
    localStorage.setItem('lancamentos', JSON.stringify(listaNova));
    setForm({
      transacao: 'despesa',
      categoria: '',
      descricao: '',
    ...
    calcularSaldo(listaNova);
  }

  function excluir(id: number) {
    const listaNova = lancamentos.filter((l) => l.id !== id);
    setLancamentos(listaNova);
    localStorage.setItem('lancamentos', JSON.stringify(listaNova));
    calcularSaldo(listaNova);
  }
  ...
  const categoriasUnicas = Array.from(
    new Set(lancamentos.map((l) => l.categoria))
  ).filter((cat) => cat);
